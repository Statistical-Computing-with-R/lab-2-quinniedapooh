...
This data has 30,463 rows and 15 columns. 
...
This dataset has characters (species_id, sex, day_of_week, plot_type, genus, species, taxa), doubles (record_id, month, day, year, plot_id, hindfoot_length, weight), and date.
...
ggplot(data = surveys,
  ...
  labs(x = "Weight",
       y="",
       title = "Relationship Between Weight & Hindfoot Length for Rodent Species",
...
With an F-test statistic of 19,916 and an incredibly small p-value of less than 2*10^-16, we can say that we have a statistically significant p-value. Thus, we can reject the null hypothesis that the population mean weight for all fourteen rodent species is the same. Furthermore, we can say that we have strong evidence that at least one rodent species has a different population mean weight from the rest of the rodent species.
